...
- `GET /api/v1/faces/list` - List one visible profile per name, sorted by latest training date, including `sample_count`, `first_trained_at`, `last_trained_at`, and `thumbnail_url` when a saved training sample exists
